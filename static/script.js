...
// FIXED SOCKET LISTENERS - Use CSS classes instead of inline colors
            socket.on('sensor_status', function(data) {
            const el = document.getElementById("gasData");
            el.innerText = data.state ? (data.state.charAt(0).toUpperCase() + data.state.slice(1)) : '--';
            el.classList.remove('good', 'working', 'error', 'fault', 'green-state', 'red-state');
            if (data.state === 'working' || data.state === 'good') {
        ...
        socket.on('gas_fault', function(data) {
            const el = document.getElementById("gasState");
            el.innerText = data.status ? (data.status.toString().charAt(0).toUpperCase() + data.status.toString().slice(1)) : '--';
            el.classList.remove('good', 'working', 'error', 'fault', 'green-state', 'red-state');
            if (data.status === 'Good' || data.status === 'good') {
                el.classList.add('good');
            } else if (data.status === 'Fault' || data.status === 'fault') {
        ...
        socket.on('gas_in', function(data) {
            const el = document.getElementById("gasIn");
            let displayValue = '--';
            if (typeof data.status === 'number') {
                displayValue = data.status + 'V';
                el.classList.remove('good', 'working', 'error', 'fault', 'green-state', 'red-state');
                if (data.status === 12) {
                ...
                displayValue = data.status ? (data.status.toString().charAt(0).toUpperCase() + data.status.toString().slice(1)) : '--';
                el.classList.remove('good', 'working', 'error', 'fault', 'green-state', 'red-state');
                if (data.status === 'Good' || data.status === 'good' || data.status === '12V') {
                    el.classList.add('green-state');
                } else if (data.status === 'Fault' || data.status === 'fault' || data.status === '0V') {
                    el.classList.add('red-state');
                }
            }
