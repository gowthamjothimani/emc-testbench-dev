...
def monitor_fuse_and_gas_n():
    ...
    while fuse_check_running:
        try:
            ...
            log_exporter.set_state("fault", "alarm_fault", alarm_fault)
            log_exporter.set_state("fault", "gas_power_in", gas_in)
...
@socketio.on('gas_fault')
def handle_gas_fault(data):
    """Receive and store gas eFuse fault state"""
    status = data.get('status', '--')
    log_exporter.set_state("fault", "gas_fault", status)

@socketio.on('badge_fault')
def handle_badge_fault(data):
    """Receive and store badge eFuse fault state"""
    status = data.get('status', '--')
    log_exporter.set_state("fault", "badge_fault", status)

@socketio.on('alarm_fault')
def handle_alarm_fault(data):
    """Receive and store alarm eFuse fault state"""
    status = data.get('status', '--')
    log_exporter.set_state("fault", "alarm_fault", status)

@socketio.on('gas_in')
def handle_gas_in(data):
    """Receive and store gas power IN state"""
    status = data.get('status', '--')
...
if __name__ == '__main__':
    ...
    heartbeat = HeartbeatPublisher(mqtt_client, interval=10, device_id=pcbserial if 'pcbserial' in globals() else None)
    heartbeat_thread = Thread(target=heartbeat.start, daemon=True)
